...
Mpg123Decoder::Mpg123Decoder(const char* data, size_t size, int bufferSize)
	: Decoder(data, size, bufferSize)
	, decoder_file(data, size)
	, handle(0)
	...
	, duration(-2.0)
{
	...
	if (!inited)
	{
		ret = mpg123_init();
		if (ret != MPG123_OK)
			throw Exception("Could not initialize mpg123.");
	...
	try
	{
		...
		if (ret != MPG123_OK)
			throw Exception("Could not set decoder callbacks.");

		ret = mpg123_open_handle(handle, &decoder_file);
		if (ret != MPG123_OK)
			throw Exception("Could not open decoder.");
		...
		if (ret == MPG123_ERR)
			throw Exception("Could not get stream information.");
		...
		if (ret != MPG123_OK)
			throw Exception("Could not read mp3 data.");
	}
	catch (Exception &)
	{
...
{
	return new Mpg123Decoder(data, size, bufferSize);
}
